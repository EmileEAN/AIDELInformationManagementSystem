...
namespace EEANWorks.WinFormsApps.AIDELInformationManagementSystem.Forms
{
    public partial class StudentCoursesForm : Form
    {
        ...
        {
            m_student = DataContainer.Instance.SelectedStudent;
            lbl_student.Text = "(" + m_student.AccountNumber.ToString() + ") " + m_student.Name.ToString();
        ...
        private void dgv_main_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (column is DataGridViewButtonColumn && rowIndex >= 0) // If it is a button column and it is not the header cell
            {
                if (column.DataPropertyName == "Group")
                {
                    ...
                    this.LogAndSwitchTo(new StudentEvaluationsForm());
                }
            }
        }
        ...
        private void InitializeMainViewTable()
        {
            ...
            // Set columns
            {
                ...
                switch (m_viewMode)
                {
                    default: // case eViewMode.OverallAveragePoints
                        {
                            columns.Add("QuantitativeEvaluationAverage", "Quantitative Evaluation Average");
                            columns.Add("QualitativeEvaluationAverage", "Qualitative Evaluation Average");
                        }
                        break;

                        //    case eViewMode.QuantitativeAveragePoints:
                        //        {
                        //            var baseCriterionReferences = m_group.Course.FullEvaluationCriteria.CriteriaSet_QuantitativeEvaluation.CriteriaWeight_QuantitativeEvaluation_FirstPartial.Criteria.WeightPerCriterion; // Referencing first partial as the template
                        //            for (int i = 0; i < baseCriterionReferences.Count; i++)
                        //            {
                        //                columns.Add("Criterion" + i.ToString(), baseCriterionReferences[i].Criterion.String);
                        //            }
                        ...
                        //            for (int i = 0; i < baseCriterionReferences.Count; i++)
                        //            {
                        //                columns.Add("Criterion" + i.ToString(), baseCriterionReferences[i].String);
                        //            }
                        //        }
                        //        break;
                        //}
        ...
        private void ApplyFilter_Main()
        {
            ...
            var courseGroupDictionary = container.CourseGroupDictionary.Where(x => x.Value.StudentInfos.Any(y => y.Student == m_student));
            ...
            var rows = dgv_main.Rows;
            {
                ...
                foreach (var element in targetDictionary)
                {
                    ...
                    Random r = new Random();
                    decimal d1 = r.Next(0, 10);
                    decimal d2 = r.Next(1, 3);
                    var row = new string[] { courseBase.Id.ToString(), courseBase.Name, semester.Term.ToString(), semester.Year.ToString(), group.Name, d1.ToString(), d2.ToString() };
                    rows.Add(row);
                }
            }
